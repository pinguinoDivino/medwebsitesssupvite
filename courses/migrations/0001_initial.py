# Generated by Django 4.1.7 on 2023-05-26 09:53

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('accounts', '0001_initial'),
    ]

    operations = [
        migrations.CreateModel(
            name='ThematicArea',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=300, unique=True, verbose_name='nome')),
            ],
            options={
                'verbose_name': 'Area tematica',
                'verbose_name_plural': 'Aree tematiche',
            },
        ),
        migrations.CreateModel(
            name='Course',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('title', models.CharField(max_length=300, verbose_name='titolo')),
                ('lecturer', models.CharField(max_length=300, verbose_name='titolare')),
                ('ssd', models.CharField(max_length=30, verbose_name='SSD titolare')),
                ('abstract', models.TextField(verbose_name='abstract')),
                ('syllabus', models.TextField(blank=True, null=True, verbose_name='syllabus')),
                ('min_participant_year', models.IntegerField(verbose_name='anno minimo raccomandato')),
                ('area', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='courses', to='courses.thematicarea', verbose_name='area tematica')),
                ('professor', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='courses', to='accounts.facultymember', verbose_name='professore')),
            ],
            options={
                'verbose_name': 'Corso interno',
                'verbose_name_plural': 'Corsi interni',
                'unique_together': {('title', 'professor')},
            },
        ),
        migrations.CreateModel(
            name='CourseAttendance',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('academic_year', models.IntegerField(choices=[[2018, '2018-2019'], [2019, '2019-2020'], [2020, '2020-2021'], [2021, '2021-2022'], [2022, '2022-2023'], [2023, '2023-2024'], [2024, '2024-2025'], [2025, '2025-2026'], [2026, '2026-2027'], [2027, '2027-2028'], [2028, '2028-2029'], [2029, '2029-2030'], [2030, '2030-2031'], [2031, '2031-2032'], [2032, '2032-2033'], [2033, '2033-2034'], [2034, '2034-2035'], [2035, '2035-2036'], [2036, '2036-2037'], [2037, '2037-2038'], [2038, '2038-2039'], [2039, '2039-2040'], [2040, '2040-2041'], [2041, '2041-2042'], [2042, '2042-2043'], [2043, '2043-2044'], [2044, '2044-2045'], [2045, '2045-2046'], [2046, '2046-2047'], [2047, '2047-2048'], [2048, '2048-2049'], [2049, '2049-2050'], [2050, '2050-2051'], [2051, '2051-2052'], [2052, '2052-2053'], [2053, '2053-2054'], [2054, '2054-2055'], [2055, '2055-2056'], [2056, '2056-2057'], [2057, '2057-2058'], [2058, '2058-2059'], [2059, '2059-2060'], [2060, '2060-2061'], [2061, '2061-2062'], [2062, '2062-2063'], [2063, '2063-2064'], [2064, '2064-2065'], [2065, '2065-2066'], [2066, '2066-2067'], [2067, '2067-2068'], [2068, '2068-2069'], [2069, '2069-2070'], [2070, '2070-2071'], [2071, '2071-2072'], [2072, '2072-2073'], [2073, '2073-2074'], [2074, '2074-2075'], [2075, '2075-2076'], [2076, '2076-2077'], [2077, '2077-2078'], [2078, '2078-2079'], [2079, '2079-2080'], [2080, '2080-2081'], [2081, '2081-2082'], [2082, '2082-2083'], [2083, '2083-2084'], [2084, '2084-2085'], [2085, '2085-2086'], [2086, '2086-2087'], [2087, '2087-2088'], [2088, '2088-2089'], [2089, '2089-2090'], [2090, '2090-2091'], [2091, '2091-2092'], [2092, '2092-2093'], [2093, '2093-2094'], [2094, '2094-2095'], [2095, '2095-2096'], [2096, '2096-2097'], [2097, '2097-2098'], [2098, '2098-2099'], [2099, '2099-2100']], verbose_name='anno accademico')),
                ('participants', models.IntegerField(verbose_name='numero partecipanti')),
                ('started_at', models.CharField(choices=[['january', 'January'], ['february', 'February'], ['march', 'March'], ['april', 'April'], ['may', 'May'], ['june', 'June'], ['july', 'July'], ['august', 'August'], ['september', 'September'], ['october', 'October'], ['november', 'November'], ['december', 'December']], max_length=40, verbose_name='mese di inizio')),
                ('ended_at', models.CharField(choices=[['january', 'January'], ['february', 'February'], ['march', 'March'], ['april', 'April'], ['may', 'May'], ['june', 'June'], ['july', 'July'], ['august', 'August'], ['september', 'September'], ['october', 'October'], ['november', 'November'], ['december', 'December']], max_length=40, verbose_name='mese di fine')),
                ('course', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='attendances', to='courses.course', verbose_name='corso interno')),
            ],
            options={
                'verbose_name': 'Dato corso',
                'verbose_name_plural': 'Dati corso',
                'unique_together': {('course', 'academic_year')},
            },
        ),
    ]
